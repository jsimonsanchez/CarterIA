...
// Genera un extracto sintético de XTB con ~20 posiciones abiertas que suman
// ~669.000 € de valor de mercado, más unas cuantas posiciones cerradas,
// dividendos, intereses e ingresos — para tomar capturas de demo sin usar
...
async function main() {
  ...
  cash.addRow(['Account number', 99999999])
  cash.addRow(['Cash Operations'])
  cash.addRow(['Date from (UTC)', new Date('2025-01-01')])
  cash.addRow(['Date to (UTC)', new Date('2026-08-24')])
  cash.addRow(['Type', 'Instrument', 'Ticker', 'Category', 'Time', 'Amount', 'ID', 'Comment', 'Product', 'Position ID'])

  let id = 2000000000
  let otherFlows = 0 // dividendos + intereses - impuestos - retenciones, para calcular el ingreso final
  ...
  for (const [i, p] of POSITIONS.entries()) {
    ...
    // Un par de dividendos por posición, con su retención.
    for (let d = 0; d < 2; d++) {
  ...
  cash.addRow(['Free funds interest', null, null, null, new Date('2026-07-06T12:30:00Z'), 145.3, id++, 'Free-funds Interest 2026-06', 'My Trades', null])
  cash.addRow(['Free funds interest tax', null, null, null, new Date('2026-07-06T12:24:00Z'), -30.5, id++, 'Free-funds Interest Tax 2026-06', 'My Trades', null])
  otherFlows += 145.3 - 30.5

  const totalCostEur = POSITIONS.reduce((acc, p) => acc + p.qty * (p.eurPrice / (1 + p.gainPct)), 0)
  const desiredLiquidity = 28450
  ...
  closed.addRow(['Account number', 99999999])
  ...
  await wb.xlsx.writeFile('public/demo-fixture.xlsx')

  const marketValue = POSITIONS.reduce((acc, p) => acc + p.qty * p.eurPrice, 0)
  console.log('market value target (EUR):', marketValue.toFixed(2))
  console.log('cost basis (EUR):', totalCostEur.toFixed(2))
  console.log('deposit needed (EUR):', depositAmount.toFixed(2))
  console.log('written public/demo-fixture.xlsx')
}
