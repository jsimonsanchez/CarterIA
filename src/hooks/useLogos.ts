 ...
 * dominio de terceros.
 *
 * Resolver un logo cuesta un crédito de la API de Twelve Data, cuyo plan
 * gratuito tiene un límite diario que se agota con facilidad. Como los logos
 * no cambian nunca, se resuelve UNA sola vez por símbolo y se guarda en
 * `symbolMappings`; a partir de ahí solo se piden los bytes de la imagen,
 * que no gastan crédito. Un símbolo sin logo se marca con `null` para no
 * volver a preguntar por él.
 */
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      for (const { symbol, mapping } of pending) {
        ...
        const resolved = await resolveLogo(mapping.twelveDataSymbol, mapping.twelveDataExchange)
...
async function resolveLogo(
  ...
  twelveDataExchange: string | undefined,
): Promise<string | null | 'retry'> {
  const params = new URLSearchParams({ symbol: twelveDataSymbol })
  if (twelveDataExchange) params.set('exchange', twelveDataExchange)
